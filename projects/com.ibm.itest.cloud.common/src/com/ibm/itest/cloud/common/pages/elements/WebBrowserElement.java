 ...
 * @param by The mechanism to use to search for the element
 */
public WebBrowserElement(final WebBrowser browser, final SearchContext context, final By by) {
 ...
 * @param by The mechanism to use to search for the element
 */
public WebBrowserElement(final WebBrowser browser, final WebBrowserFrame webFrame, final SearchContext context, final By by) {
 ...
 * used when searching for several element (see {@link #findElements(By, boolean, boolean)})
 */
public WebBrowserElement(final WebBrowser browser, final WebBrowserFrame webFrame, final SearchContext context, final By by, final WebElement element, final int size, final int index) {
	super();
...
public boolean equals(final Object obj) {
	return (obj instanceof WebBrowserElement) && this.webElement.equals(((WebBrowserElement)obj).webElement);
}
 ...
 */
@Override
public WebBrowserElement findElement(final By findBy) {
 ...
 * @throws ScenarioFailedError If the element is not found.
 */
public WebBrowserElement findElement(final By findBy, final boolean recovery) {
...
public List<WebElement> findElements(final By locator, final boolean displayed, final boolean recovery) {
	...
	//				if (foundElement.isDisplayed() || !displayed) {
					if (!displayed || foundElement.isDisplayed()) {
						WebBrowserElement webPageElement = new WebBrowserElement(this.browser, this.frame, this, fixedLocator, foundElement, size, idx);
	...
	//						}
				...
				return WebBrowser.NO_ELEMENT_FOUND;
			}
			catch (UnhandledAlertException uae) {
				this.browser.purgeAlerts("Finding element '"+fixedLocator+"'");
				if (!recovery) {
...
public String getFullPath() {
	...
	if (this.context instanceof WebBrowserElement) {
		xpathBuilder.append(((WebBrowserElement) this.context).getFullPath());
	}
	...
	try {
		...
		if (this.parentListSize == 0) {
			...
			if (this.context instanceof WebBrowserElement) {
				...
				try {
					recoveredElement = ((WebBrowserElement) this.context).webElement.findElement(this.by);
				}
				catch (NoSuchElementException nsee) {
					if (n == MAX_RECOVERY_ATTEMPTS) {
			...
			if (this.context instanceof WebBrowserElement) {
				...
				foundElements = ((WebBrowserElement) this.context).webElement.findElements(this.by);
			} else {
 ...
 * only one was expected.
 */
public WebBrowserElement waitForElement(final By locator, final int timeout, final boolean displayed, final boolean single) {
	if (DEBUG) debugPrintln("		+ waiting for element: ["+locator+"] relatively to "+this);

	// Wait for all elements
	List<WebBrowserElement> foundElements = waitForElements(locator, timeout, displayed);
