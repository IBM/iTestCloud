 ...
 * <ul>
 * <li>{@link #createFolderElement(WebBrowserElement)}: Creates the folder element instance from the given web element.</li>
 ...
 * </ul>
 */
public class WebFolderElement extends WebElementWrapper {
...
//	final private WebFolderElement parent;
	 ...
	 */
	protected AbstractExpandableElement expandableElement;
...
public WebFolderElement(final WebPage page, final By elementBy, final By expandBy, final By expansionBy, final By containerBy) {
	this(page, elementBy, expandBy, expansionBy, containerBy, null);
}

public WebFolderElement(final WebPage page, final By elementBy, final By expandBy, final By expansionBy, final By containerBy, final WebFolderElement parentFolder) {
...
public WebFolderElement(final WebPage page, final WebBrowserElement element, final By expandBy, final By expansionBy, final By containerBy) {
	this(page, element, expandBy, expansionBy, containerBy, null);
}

public WebFolderElement(final WebPage page, final WebBrowserElement element, final By expandBy, final By expansionBy, final By containerBy, final WebFolderElement parentFolder) {
	super(page, element);
	this.parent = parentFolder;
	this.expandBy = expandBy;
	this.expansionBy = expansionBy;
...
public WebFolderElement(final WebPage page, final WebBrowserElement element, final WebFolderElement parentFolder) {
	super(page, element);
	this.parent = parentFolder;
 ...
 * @return The folder element as a {@link WebFolderElement}.
 */
protected WebFolderElement createFolderElement(final WebBrowserElement childElement) {
	return new WebFolderElement(getPage(), childElement, this);
}
...
private WebFolderElement find(final String parentName, final String folder, final boolean leaf) {

	// Get the children
	List<WebFolderElement> children = getChildren();

	// Look for the searched folder
	for (WebFolderElement child : children) {
		...
		if (container) {
	    	WebFolderElement folderElement = child.find(parentName, folder, leaf);
 ...
 * @return The matching child folder as a {@link WebFolderElement} or
 * <code>null</code> if current folder has no child with the given name.
 */
public WebFolderElement findFolder(final String folder) {
 ...
 * @return The matching folder as a {@link WebFolderElement} or
 * <code>null</code> if none was found.
 */
public WebFolderElement findFolder(final String parentFolder, final String folder) {
 ...
 * @return The matching leaf as a {@link WebFolderElement} or
 * <code>null</code> if none was found.
 */
public WebFolderElement findLeaf(final String parentFolder, final String leaf) {
 ...
 *
 * @return All folder leaves as a {@link List} of {@link WebFolderElement}.
 */
public List<WebFolderElement> getAllLeaves() {

	// Init list
	List<WebFolderElement> allLeaves = new ArrayList<WebFolderElement>();
 ...
 * <code>null</code> if current folder has no child with the given name.
 */
protected WebFolderElement getChild(final String folder) {
	// Get the children.
	List<WebFolderElement> children = getChildren();
	for (WebFolderElement child : children) {
		String childFolderName = child.getText();
 ...
 *
 * @return The list of children as a {@link List} of {@link WebFolderElement}.
 */
final public List<WebFolderElement> getChildren() {
	// Expand node first
	expand();
	// Look for children elements
	if (getContainerElement().isDisplayed()) {
		List<WebBrowserElement> childrenElements = getChildrenElements();
		List<WebFolderElement> childrenFolderElements = new ArrayList<WebFolderElement>(childrenElements.size());
 ...
 *
 * @return The web element as a {@link WebBrowserElement}.
 */
final protected AbstractExpandableElement getExpandableElement() {
 ...
 * @return The selected folder as a {@link WebFolderElement} or
 * <code>null</code> if no folder was not found for the given folder path.
 */
public WebFolderElement getFolder(final String folderPath) {
	...
	WebFolderElement childFolder = getChild(childName);
 ...
 *
 * @return The folder as a {@link WebFolderElement} or <code>null</code>
 * if current folder is the root.
 */
public WebFolderElement getParentFolder() {
	return (WebFolderElement) this.parent;
}
...
protected void initExpandableElement() {
	WebBrowserElement expandElement = waitForElement(this.expandBy, timeout());
	this.expandableElement = new AbstractExpandableElement(getPage(), expandElement, this.expansionBy);
}
 ...
 * @return The first folder matching the given name as a {@link WebFolderElement}
 * or <code>null</code> if none was found.
 */
public WebFolderElement searchFolder(final String folder) {
	...
	for (WebFolderElement child : children) {
		if (child.isContainer()) {
	    	WebFolderElement searchedFolder = child.searchFolder(folder);
 ...
 * @param folderPath The folder path. Might be a simple name or a fully-qualified
 * path using '/' character for segment delimiter.
 * @return The selected folder as a {@link WebFolderElement}.
 * @throws ScenarioFailedError if no folder was not found for the given folder path.
 */
public WebFolderElement selectFolder(final String folderPath) throws ScenarioFailedError {
	WebFolderElement folderElement = getFolder(folderPath);
	if (folderElement == null) {
