 ...
 * <ul>
 * <li>{@link #createChildElement(WebBrowserElement)}: Create a element wrapper for a given child tree node or tree leaf element.</li>
 ...
 * </p>
 */
public class WebTreeNodeElement extends AbstractExpandableElement {
 ...
 */
public WebTreeNodeElement(final WebElementWrapper parent, final By locator, final By labelLocator, final By expansionLocator, final By childrenLocator) {
 ...
 *
 * @param parent The web element containing the tree node element.
 * @param webElement The tree node element as {@link WebBrowserElement}.
 * @param labelLocator The xpath to locate the label element of the tree node element as {@link By}.
 * This xpath must be relative to the tree node element.
 * @param expansionLocator The xpath to locate the element used to expand or collapse the tree node
 * element as {@link By}.This xpath must be relative to the tree node element.
 ...
 * as {@link By}. This xpath must be relative to the tree node element.
 */
public WebTreeNodeElement(final WebElementWrapper parent, final WebBrowserElement webElement, final By labelLocator, final By expansionLocator, final By childrenLocator) {
 ...
 * as {@link By}. This xpath must be relative to the tree node element.
 */
public WebTreeNodeElement(final WebPage page, final By locator, final By labelLocator, final By expansionLocator, final By childrenLocator) {
	super(page, locator);
	this.labelLocator = labelLocator;
	this.expansionLocator = expansionLocator;
	this.childrenLocator = childrenLocator;
 ...
 * as {@link By}. This xpath must be relative to the tree node element.
 */
public WebTreeNodeElement(final WebPage page, final WebBrowserElement webElement, final By labelLocator, final By expansionLocator, final By childrenLocator) {
...
public void collapseAll() {
	if(isExpanded()) {
		for (WebTreeNodeElement childElement : getChildElements()) {
 ...
 * </p>
 *
 * @param childWebElement The child tree or tree leaf element as {@link WebBrowserElement}.
 *
 * @return The element wrapper for the given child tree node or tree leaf element as {@link WebTreeNodeElement}.
 */
protected WebTreeNodeElement createChildElement(final WebBrowserElement childWebElement) {
	return new WebTreeNodeElement(this.page, childWebElement, this.labelLocator, this.expansionLocator, this.childrenLocator);
}
 ...
 */
protected List<WebTreeNodeElement> getAllChildElements() {
	if(isExpandable()) expand();

	List<WebTreeNodeElement> allChildElements = new ArrayList<WebTreeNodeElement>();
	List<WebTreeNodeElement> childElements = getChildElements();

	for (WebTreeNodeElement childElement : childElements) {
 ...
 * @return The child tree node or tree leaf element at the given path as {@link WebTreeNodeElement} or
 * <code>null</code> if a matching element is not found and asked not to fail.
 */
protected WebTreeNodeElement getChildElement(final String path, final boolean fail) {
 ...
 */
protected List<WebTreeNodeElement> getChildElements() {
	List<WebTreeNodeElement> childWebFolderElements = new ArrayList<WebTreeNodeElement>();
	List<WebBrowserElement> childWebElements = this.element.waitForElements(this.childrenLocator, tinyTimeout(), false /*displayed*/);

	for (WebBrowserElement childWebElement : childWebElements) {
 ...
 *
 * @return The web element used to expand or collapse the tree node element as {@link WebBrowserElement}.
 */
protected WebBrowserElement getExpansionElement() {
 ...
 *
 * @return The label element of the tree node or tree leaf element as {@link WebBrowserElement}.
 */
public WebBrowserElement getLabelElement() {
...
public boolean isExpanded() throws ScenarioFailedError {
	for (WebTreeNodeElement webTreeNodeElement : getChildElements()) {
