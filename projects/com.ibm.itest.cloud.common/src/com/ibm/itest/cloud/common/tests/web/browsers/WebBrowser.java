 ...
 * </p>
 */
public abstract class WebBrowser implements SearchContext {
...
//	protected static final String ELEMENT_SCROLL_BEHAVIOR_VALUE = "1";
	...
	public static final List<WebBrowserElement> NO_BROWSER_ELEMENT_FOUND = new ArrayList<WebBrowserElement>();
     ...
     *
     * @return A specialized instance of {@link WebBrowser}.
     */
	...
	protected WebBrowserFrame framePopup;
 ...
 * clicked.
 */
public WebBrowserElement clickButton(final WebBrowserElement button, final int timeout, final boolean validate) {
 ...
 * @see Actions#dragAndDrop(WebElement, WebElement)
 */
public void dragAndDropViaJavaScript(final WebBrowserElement sourceElement, final WebBrowserElement targetElement) {
	debugPrintln("		+ Drag " + sourceElement + " to " + targetElement);
	dragAndDropViaJavaScript(sourceElement, targetElement, Position.Top_Left, Position.Top_Left);
}

@SuppressWarnings("boxing")
private void dragAndDropViaJavaScript(final WebBrowserElement dragFrom, final WebBrowserElement dragTo, final Position dragFromPosition, final Position dragToPosition) {
 ...
 * @return The found element as {@link WebBrowserElement} or <code>null</code>.
 */
 ...
 */
public WebBrowserElement[] select(final WebBrowserElement listElement, final By entriesBy, final boolean useControl, final Pattern... patterns) {
	WebBrowserElement[] selectedElements = new WebBrowserElement[patterns.length];
	List<WebBrowserElement> entryElements = listElement.waitForElements(entriesBy);

	outer:
	for(int i = 0; i < patterns.length; i++) {
		for(WebBrowserElement entryElement : entryElements) {
 ...
 * having retried {@link WebBrowserElement#MAX_RECOVERY_ATTEMPTS} times.
 */
public WebBrowserElement[] select(final WebBrowserElement listElement, final By entriesBy, final boolean useControl, final String... items) {
 ...
 * having retried {@link WebBrowserElement#MAX_RECOVERY_ATTEMPTS} times.
 */
public WebBrowserElement[] select(final WebBrowserElement listElement, final By entriesBy, final Pattern... patterns) {
 ...
 */
public void selectEmbeddedFrame(final WebBrowserElement frameElement, final WebBrowserFrame browserFrame) {
 ...
 * @param timeout The time in seconds to wait before giving up the research.
 */
public void selectFrame(final WebBrowserElement parentElement, final By locator, final int timeout) {
	if (DEBUG) {
		debugPrintln("		+ select frame " + locator);
		debugPrintln("		  -> current frame: "+getCurrentFrame());
	}
 ...
 */
public void selectMultipleElements(final WebBrowserElement... elements) {
 ...
 * @since 6.0
 */
public void typePassword(final WebBrowserElement element, final int timeout, final IUser user) {
...
private void typeText(final WebBrowserElement element, final String text, final Keys key, final int keyDelay, final boolean clear, final int timeout, final IUser user) {
	String printedText;
	if (user == null) {
 ...
 * @throws MultipleVisibleElementsError if several elements are found and only single one was expected
 */
public WebBrowserElement waitForElement(final WebBrowserElement parentElement, final By locator, final boolean fail, final int timeout) {
 ...
 */
public boolean waitWhileDisplayed(final WebBrowserElement parentElement, final By locator, final int seconds, final boolean fail) {
 ...
 * given timeout has been reached and it has been asked to fail.
 */
public boolean waitWhileDisplayed(final WebBrowserElement parentElement, final By locator, final int seconds, final boolean fail, final boolean single) {
	WebBrowserElement element = waitForElement(parentElement, locator, false /*fail*/, 1 /*timeout*/, true /*displayed*/, single);
