...
sudo apt-get update || errorExit "Enabling update repository failed, exiting"
...
then
	...
	wget -nv https://ftp.mozilla.org/pub/firefox/releases/78.6.0esr/linux-x86_64/en-US/firefox-78.6.0esr.tar.bz2 || errorExit "Downloading Firefox failed, exiting"

	echo -e ">>>>>>>>>>>>>> Extracting Firefox"
	tar -xjf firefox-78.6.0esr.tar.bz2 || errorExit "Extracting Firefox failed, exiting"

	echo -e ">>>>>>>>>>>>>> Downloading Gecko driver archive"
	wget -nv https://github.com/mozilla/geckodriver/releases/download/v0.28.0/geckodriver-v0.28.0-linux64.tar.gz || errorExit "Downloading Gecko driver archive failed, exiting"

	echo -e ">>>>>>>>>>>>>> Extracting Gecko driver archive"
	tar -xzf geckodriver-v0.28.0-linux64.tar.gz || errorExit "Extracting Gecko driver archive failed, exiting"
