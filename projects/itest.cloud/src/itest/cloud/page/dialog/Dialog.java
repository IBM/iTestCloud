 ...
 * <li>{@link #closeByOpeningElement(Class, String...)}: Close the dialog by opening an element.</li>
 ...
 * </ul>
  * </p>
 */
abstract public class Dialog extends AbstractWindow {

	private List<BrowserElement> alreadyOpenedDialogElements;

public Dialog(final Page page, final By findBy) {
	super(page, findBy);
}

public Dialog(final Page page, final By findBy, final String... data) {
	super(page, findBy, data);
}

public Dialog(final Page page, final By findBy, final String frame) {
...
/**
 * Close the dialog by opening an element.
 *
 * @param validate Specifies whether the close action is to validate or to cancel.
 * @param elementClass A class representing the element opened after closing the dialog.
 * @param elementData Additional information to store in the element when opening it.
 *
 * @return The opened element as a {@link ElementWrapper}.
 */
<T extends ElementWrapper> T closeByOpeningElement(final boolean validate, final Class<T> elementClass, final String... elementData) {
	// Perform any actions prior to closing the window.
	if(validate) preCloseActions();

	return openElementUsingLink(waitForElement(By.xpath(getCloseButton(validate))), elementClass, elementData);
}
 ...
 * @return The opened element as a {@link ElementWrapper}.
 */
public <T extends ElementWrapper> T closeByOpeningElement(final Class<T> elementClass, final String... elementData) {
	return closeByOpeningElement(true /*validate*/, elementClass, elementData);
}
...
private List<BrowserElement> getMatchingDialogElements(final List<BrowserElement> openedDialogElements) {
	...
	for (BrowserElement openedDialogElement : openedDialogElements) {
		...
		if((titleElement != null) && getExpectedTitle().matcher(getTitle(titleElement)).matches()) {
 ...
 *
 * @return The dialog as a {@link Dialog} subclass.
 */
	...
	while(true) {
		// Get list of opened dialog elements
	   	...
	   	if (System.currentTimeMillis() > timeoutMillis) {
			throw new WaitElementTimeoutError("A mathcing opened dialog could not be found before the timeout '" + timeout() + "'s had reached.");
		}
	}
}

/**
 * Wait for the content element to load.
 */
protected void waitForContentElement() {
	final By ContentElementLocator = getContentElementLocator();
	if(ContentElementLocator != null) waitForElement(getContentElementLocator());
}
