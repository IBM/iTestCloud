...
import static itest.cloud.utils.ByUtils.isRelativeLocator;
...
public DropdownlistElement(final ElementWrapper parent) {
	super(parent);
	this.selectionLocator = null;
}

public DropdownlistElement(final ElementWrapper parent, final By locator, final By expansionLocator, final By selectionLocator) {
	super(parent, locator, expansionLocator);
	this.selectionLocator = selectionLocator;
}

public DropdownlistElement(final ElementWrapper parent, final BrowserElement webElement, final By expansionLocator, final By selectionLocator) {
	super(parent, webElement, expansionLocator);
	this.selectionLocator = selectionLocator;
}

public DropdownlistElement(final Page page) {
	super(page);
	this.selectionLocator = null;
}

public DropdownlistElement(final Page page, final By findBy, final By expansionLocator, final By selectionLocator) {
	super(page, findBy, expansionLocator);
	this.selectionLocator = selectionLocator;
}

public DropdownlistElement(final Page page, final BrowserElement webElement, final By expansionLocator, final By selectionLocator) {
 ...
 *
 * @param optionsItemElement The option element as {@link BrowserElement}.
 *
 * @return The label associated with a given option element as {@link String}.
 */
protected String getOptionElementLabel(final BrowserElement optionsItemElement) {
...
protected List<BrowserElement> getOptionElements(final boolean fail) {
	...
	return waitForElements(parentElement, optionElementLocator, (fail ? timeout() : tinyTimeout()), fail, false /*displayed*/);
}
	...
	try {
		if(findBy != null) {
			if((data == null) || (data.length == 0)) {
				...
				confirmationDialog = dialogClass.getConstructor(Page.class, By.class, String[].class).newInstance(getPage(), findBy, data);
			}
		}
		else {
			if((data == null) || (data.length == 0)) {
