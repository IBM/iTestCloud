...
import static itest.cloud.utils.StringComparisonCriterion.EQUALS;
...
import itest.cloud.utils.StringComparisonCriterion;
 ...
 * <li>{@link #open(BrowserElement)}: Open the rich hover by flying over
 ...
 * </ul>
  * </p>
 */
abstract public class LinkHoverElement<P extends Page> extends TextHoverElement {
...
public LinkHoverElement(final Page page) {
	super(page);
}
 ...
 * </p>
 * @return The opened page after the click as a subclass of {@link Page}
 */
public P clickOnTitle() {
	...
	// Get title element
	BrowserElement titleElement = getTitleElement();
 ...
 *
 * @return The hover title element as {@link BrowserElement}
 * @throws ScenarioFailedError If the title element is not found
 */
public BrowserElement getTitleElement() {
	// Get the title element
	BrowserElement hoverTitleElement = waitForElement(By.xpath(getTitleLinkXpath()), tinyTimeout(), false /*fail*/);
	if (hoverTitleElement == null) {
		// Check if there's a login
		BrowserElement loginElement = waitForElement(By.xpath(".//a[text()='Log in']"), tinyTimeout(), false /*fail*/);
		if (loginElement != null) {
			// Login
			// TODO Need improvement for distributed topology
			loginElement.click();
			// Wait again for the title
			hoverTitleElement = waitForElement(By.xpath(getTitleLinkXpath()), tinyTimeout(), false /*fail*/);
		}
		if (hoverTitleElement == null) {
			// Wait again for the title
			hoverTitleElement = waitForElement(By.xpath(getTitleLinkXpath()), tinyTimeout(), false /*fail*/);
	...
	while (this.title.isEmpty()) {
		...
		// Wait again for the title
		hoverTitleElement = waitForElement(By.xpath(getTitleLinkXpath()), openTimeout());
		this.title = hoverTitleElement.getText();
	}
