...
import static itest.cloud.utils.FileUtil.createDir;
...
import itest.cloud.scenario.errors.InvalidCommandException;
import itest.cloud.scenario.errors.ScenarioFailedError;
import itest.cloud.utils.EncryptionUtils;
...
public static String getDecryptedParameterValue(final String name, final String defaultEncryptedValue) {
	...
	String value = EncryptionUtils.decrypt(encryptedValue);
...
public static void printStackTrace(final int tabs) {
    ...
    for (StackTraceElement element: elements) {
    	if (first) {
    		...
    		String elementClassName = element.getClassName();
    		if (elementClassName.startsWith("com.ibm.itest.cloud")) {
	    		printIndent(tabs);
...
public static void printStackTrace(final StackTraceElement[] elements, final int tabs) {
    for (StackTraceElement element: elements) {
		String elementClassName = element.getClassName();
		if (elementClassName.startsWith("com.ibm.itest.cloud") && !elementClassName.startsWith("com.ibm.itest.cloud.common.tests.scenario.Scenario")) {
    		printIndent(tabs);
    		println(element.toString());
    	}
    }
...
private static Properties readParametersFile(final String fileDir, final String filePath) {
	...
	if (paramFile.exists()) {
		try {
			final FileInputStream stream = new FileInputStream(paramFile);
			println("The parameters properties file '"+paramFile.getPath()+"' has been found.");
			try {
...
public static List<String> toStrings(final List<? extends WebElement> elements) {
	return ScenarioUtils.toStrings(elements, false);
}
